(function (context) {
	...
	// Laconic DOM Creation attributed to Joe Stelmach https://github.com/joestelmach/laconic
				...
				for(var key in arg) {
					if(arg.hasOwnProperty(key)) {
						...
						if(value !== null && value !== undefined) {
							...
							// Allow for setting innerHTML and innerText
							else if(key === 'innerHTML' || key === 'innerText') {
								el[key] = value;
							}

							// Allow for attaching to parentNode
							else if(key === 'parentNode' && value.nodeType === 1) {
	...
	for (var i=0; i<tags.length; i++) {
		savvy.Dom[tags[i]] = makeApply(tags[i]);
	}
